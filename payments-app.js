...
(function(){
    ...
    try {
      ...
      // Try Netlify function for STK push
        try {
          ...
          const stkUrl = useLocalBackend ? 'http://localhost:5000/.netlify/functions/payment-intent' : '/.netlify/functions/payment-intent';
      ...
      // Fallback to hosted checkout via Netlify function
        try {
        const useLocalBackend = (new URLSearchParams(window.location.search).get('useLocalBackend') === '1') && (location.hostname === '127.0.0.1' || location.hostname === 'localhost') && location.port === '5500';
        const createUrl = useLocalBackend ? 'http://localhost:5000/.netlify/functions/create-payment' : '/.netlify/functions/create-payment';
  ...
  async function verifyPaymentsApi() {
    try {
      const j = await apiFetch('/api/payments/debug');
      console.log('[Payments Debug]', j);
    } catch (e) {
      console.warn('Payments API debug failed:', e.message);
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-select via query params from Quick Deposit
    try {
      ...
      if (method === 'mpesa') {
        ...
        if (amtEl && !isNaN(amt) && amt >= 10) amtEl.value = String(amt);
        if (phoneEl && phone) phoneEl.value = phone;
        // Only auto-trigger if user is logged in and Notification permission state is handled
        const isLogged = storage.isLoggedIn && storage.isLoggedIn();
        if (auto && isLogged) {
          // Slight delay to allow UI render
          setTimeout(() => initiateStkPush(), 200);
        }
      }
    } catch {}
  });
