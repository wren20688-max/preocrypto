...
async function updateUserBalance(identifier, balanceType, newBalance) {
  const db = getSupabase();
  if (!db) {
    if (memoryDB.users[identifier]) {
      if (balanceType === 'demo') {
        memoryDB.users[identifier].demo_balance = newBalance;
      } else {
        memoryDB.users[identifier].real_balance = newBalance;
      }
      return memoryDB.users[identifier];
    }
    return null;
  }
  ...
  const { data, error } = await db
    ...
    .or(`id.eq.${identifier},email.eq.${identifier},username.eq.${identifier}`)
    .select()
    .single();
...
async function updateUser(identifier, updates) {
  ...
  if (!db) {
    // Update in memory
    const user = memoryDB.users[identifier];
    if (user) {
  ...
  // Update in Supabase by email or username
  const { data, error } = await db
    .from('users')
    .update(updates)
...
async function updateUserRole(identifier, role) {
  return await updateUser(identifier, { role });
}
