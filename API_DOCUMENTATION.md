...
These serverless endpoints enable Claude Sonnet 4.5 for all clients via Vercel environment variables.
...
> Note: Do not commit secrets to the repo. Configure them in Vercel → Settings → Environment Variables and redeploy.

### 16. Get Active AI Model
```http
GET /api/model
```
...
{
  "messages": [
    { "role": "user", "content": "Write a short market summary." }
  ],
  "max_tokens": 256
}
```
...
{ "prompt": "Write a short market summary.", "max_tokens": 256 }
```
...
**Errors:**
- `400` if `AI_PROVIDER` is not `anthropic`
